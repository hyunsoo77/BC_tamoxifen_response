...
add_worksheet_gsea_results <- function(wb, res, collections=NULL, col_log2fc="log2FoldChange", col_pvalue="pvalue", col_padj="padj", th_log2fc=log2(2), th_padj=0.05, th_gsea_padj=0.05, log_cmd="display", log_cmd_txt="display_html") {
  ...
  for (collection in collections) {
    ...
    if (!collection %in% names(list_gmt)) next
...
# write_xlsx(wb, sprintf("xlsx/%s.xlsx", str_condition))
add_worksheet_enricher_results <- function(wb, res, universe=NULL, collections=NULL, col_log2fc="log2FoldChange", col_pvalue="pvalue", col_padj="padj", th_log2fc=log2(2), th_padj=0.05, th_enricher_padj=0.05, th_enricher_qval=0.05, log_cmd="display", log_cmd_txt="display_html") {
  ...
  for (collection in collections) {
    ...
    if (length(gene_symbols) > 5) {
      ...
      #log_txt(sprintf("\tn_universe=%d\n", length(universe)))
      #log_obj(head(universe))
...
#
execute_enrichment_analysis <- function(obj, str_condition=NULL, collections=NULL, col_log2fc="avg_log2FC", col_pvalue="p_val", col_padj="p_val_adj", th_log2fc=th_log2fc_global, th_padj=th_padj_global, th_gsea_padj=0.05, th_enricher_padj=0.05, th_enricher_qval=0.05, method_dge="seurat_findmarkers_enricher", dir_xlsx="xlsx", n_log=0, log_cmd="display", log_cmd_txt="display_html") {
